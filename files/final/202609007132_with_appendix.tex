...
热风烘干是中药材加工的重要环节，烘房环境通过药材表面向内部传热，同时推动内部水分向外迁移。为描述这一过程，本文根据药材细长圆柱的形状特征，忽略轴向传递和端面影响，建立\textbf{一维轴对称径向传热传质模型}。烘房温度与水分浓度由附件数据插值得到，药材表面采用换热、传质边界。四个问题依次考虑\textbf{常物性预热}、\textbf{变物性热湿耦合}、固定半径下的干燥达标以及半径收缩后的干燥达标，空间上统一采用\textbf{圆柱径向有限体积法}，时间上采用\textbf{自适应 BDF 方法}，从而在同一框架内逐步加入物性变化、终止判据和移动边界。
