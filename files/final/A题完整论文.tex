% !TeX program = xelatex
% A题完整论文（四问）：基于仓库 results/problem1--4 与 code/problem1--4 整理；前三问承接 agent_B 初稿，问题四（移动边界·半径收缩·附录4）由 agent_A 补全。
...
\noindent \textbf{针对问题四，}在问题二、问题三耦合模型的基础上引入药材失水收缩的移动边界，物性改用附录4经验公式。半径 $R(t)$ 由附件2给定（从 $2.0\rm\,cm$ 单调收缩到 $1.198\rm\,cm$），采用 Landau 前置固定变换 $\xi=r/R(t)$ 将收缩物理域映射到固定计算域 $[0,1]$；因干基含水率为物质属性且干基质量守恒，取物质坐标形式，$\xi$ 系方程不含网格对流项，收缩仅通过 $1/R^2$ 缩短扩散路径加速干燥。计算得到烘干终止时间为 $182350\rm\,s$，即 $50.6528\rm\,h$，短于问题三固定半径的 $55.0583\rm\,h$，说明半径收缩显著加快了后期达标。

\noindent \textbf{关键词：}热风烘干\quad 径向有限体积法\quad 热质耦合\quad Picard 迭代\quad 移动边界\quad 干基含水率
...
\begin{equation}
  ...
  \qquad t_j\le t\le t_{j+1},
\end{equation}
其中附件2覆盖 $0$ 到 $72\rm\,h$、间隔 $1800\rm\,s$，$R$ 从 $2.0\rm\,cm$ 单调降至 $1.198\rm\,cm$；当 $t>72\rm\,h$ 时取末值 $1.198\rm\,cm$（本实现 $t_{\rm end}=50.65\rm\,h<72\rm\,h$，未触发外推）。$R(t)$ 是给定的经验数据而非求解量，在每个时间步由上式取值后用于移动边界变换。
...
\subsection{模型求解}
问题四在固定 $\xi$ 网格上采用全隐式 backward-Euler 推进，生产配置取 $N=201$ 个 $\xi$ 节点（$\Delta\xi=0.005$，对应 $R=2\rm\,cm$ 时物理间距 $0.01\rm\,cm$，细于 $0.1\rm\,cm$ 输出分辨率），时间步 $\Delta t=10\rm\,s$。$T$ 与 $C$ 的双向耦合仍用 Picard 迭代联立，容差 $10^{-8}$、上限 $8$ 次。$\xi$ 系方程等价于单位 $\xi$-圆盘上的径向扩散（有效扩散系数 $\mathrm{cond}/R^2$、有效对流 $\mathrm{transfer}/R$），故直接复用问题二、问题三的守恒径向有限体积隐式算子；中心 $\xi=0$ 用对称性消除 $1/\xi$ 奇点，表面 $\xi=1$ 施加第三类边界，三对角系统用 Thomas 算法求解。每步由附件2插值更新 $R(t)$，并对 $C$ 取 $\max(C,0)$ 防止后期指数衰减下的负值。

\begin{algorithm}[H]
  \SetAlgoLined
  \KwIn{初值 $T_0,C_0$，附录4物性，附件1环境边界及末值外推，附件2半径序列 $R(t)$}
  \KwOut{烘干结束时间 $t_{\rm end}$ 与含移动表面列的水分浓度过程表}
  构造固定 $\xi$ 网格，初始化 $t=0$、$C(\xi,0)=2.55$\;
  \While{$\max_\xi C(\xi,t)\ge0.15$}{
    由附件2插值取当前 $R(t)$\;
    按 Picard 迭代联立求解 $\xi$ 系压缩扩散方程得 $T,C$\;
    若到达输出间隔，按固定距离 $d=\xi R(t)$ 记录水分浓度，$d>R(t)$ 处置空\;
    更新 $t$\;
  }
  输出 $t_{\rm end}$ 及终止时刻水分浓度\;
...
完整求解中 $18235$ 个时间步全部耦合收敛，单步最大 Picard 迭代次数为 $7$（上限 $8$），说明移动边界与附录4强非线性耦合在该步长下仍能稳定求解。

\subsection{结果分析}
计算得到
\begin{equation}
  t_{\rm end}=182350\rm\,s=50.6528\rm\,h.
\end{equation}
半径从首次采样时刻的 $1.9958\rm\,cm$ 收缩到终止时刻的 $1.2000\rm\,cm$。终止时刻中心全精度含水率为 $0.149998\rm\,kg/kg<0.15$，表中显示 $0.1500$ 为四舍五入结果。表\ref{tab:q4-moist} 给出每 $6\rm\,h$ 及终止时刻、不同固定距离与移动表面处的水分浓度；由于半径收缩，当固定距离大于当前半径 $R(t)$ 时该处已无药材，相应单元置空（如 $1.5\rm\,cm$ 列在 $6\rm\,h$ 起全程为空，因 $R(6\rm\,h)=1.374\rm\,cm<1.5$）。
...
\begin{table}[H]
  ...
  \begin{tabular}{rrrrrr}
    ...
    50.6528 & 0.1500 & 0.1412 & 0.1080 &  & 0.0514\\
    \bottomrule
  \end{tabular}
\end{table}

与问题三固定半径的 $55.0583\rm\,h$ 相比，问题四引入半径收缩后终止时间缩短为 $50.6528\rm\,h$。虽然附录4的扩散基值更小，但收缩经 $1/R^2$（末端约 $2.78$ 倍）显著缩短扩散路径、加快后期干燥，净效果是更早达标。表面含水率快速平衡到约 $0.0514\rm\,kg/kg$（接近环境末值），中心含水率单调下降并控制终止时刻，物理规律与前三问一致。

\subsection{本问小结与后续衔接}
问题四用物质坐标前置固定把收缩移动边界化为固定域上的压缩扩散，复用问题二、问题三的耦合有限体积算子，得到附件1末值外推与附录4物性下的终止时间 $50.6528\rm\,h$，并量化了半径收缩相对固定半径基准的加速效果。该结果与问题三共同构成模型检验与评价中收缩项影响的证据。
...
问题四在物质坐标网格上做网格与时间步无关性验证。固定 $\Delta t=10\rm\,s$、$\xi$ 点数 $N$ 由 $21$ 加密到 $401$ 时，终止时间 $t_{\rm end}$ 依次为 $49.7389$、$50.2694$、$50.5083$、$50.6528$、$50.7361\rm\,h$，相邻增量 $+0.53/+0.24/+0.14/+0.08\rm\,h$ 递减，$N=201$ 已落在 $N=401$ 的 $0.2\%$ 内；固定 $N=101$、$\Delta t$ 取 $5/10/20\rm\,s$ 时 $t_{\rm end}$ 分别为 $50.5056/50.5083/50.5167\rm\,h$，差异小于 $0.01\rm\,h$。故生产配置取 $N=201$、$\Delta t=10\rm\,s$，结果对网格与时间步不敏感。问题四 $18235$ 个时间步全部耦合收敛，单步最大 Picard 迭代 $7$ 次。

\subsection{边界敏感性讨论}
问题三、问题四中 $4\rm\,h$ 后的烘房温湿边界并非由附件直接给出，而是按附件1末值恒定外推。由于问题三 $t_{\rm end}=55.0583\rm\,h$、问题四 $t_{\rm end}=50.6528\rm\,h$，大部分积分时间都处于外推边界下，因此二者终止时间均应表述为“在附件1末值恒定外推假设下”的结果。问题四另有 front-fixing 口径（物质型无对流项）与固定距离列空值处理两项建模选择，其中物质型口径已通过 $D=0$ 时物质剖面守恒的自检。
...
\begin{itemize}[leftmargin=2em]
  \item 模型从问题一到问题四保持同一径向守恒框架，能够自然衔接预热阶段、三小时窗口、达标停机与移动边界收缩问题。
  ...
  \item 问题四用物质坐标前置固定把收缩移动边界化为固定域压缩扩散，复用前三问的耦合有限体积算子，并以 $D=0$ 守恒自检验证 front-fixing 口径无虚假平流。
\end{itemize}

\subsection{模型不足}
\begin{itemize}[leftmargin=2em]
  \item 模型未显式加入蒸发潜热源项，原因是题目未提供相变潜热及其耦合参数；该简化可能影响温度与水分迁移之间的真实耦合强度。
  \item 问题三、问题四 $4\rm\,h$ 后边界采用附件1末值恒定外推，终止时间对该外推假设可能敏感。
  \item 问题二主结果网格与题目输出网格一致，尚缺更细空间网格和相邻时间步的正式对照表。
  \item 问题四的半径 $R(t)$ 取附件2经验数据、front-fixing 采用物质型口径，空间型口径与外推边界对 $t_{\rm end}$ 的影响尚未系统量化。
\end{itemize}

\subsection{模型改进方向}
后续可从四方面改进：第一，若题目补充恒温干燥段设定值，应替换末值外推边界并重算问题三、问题四；第二，补充 $T_\infty$、$C_\infty$、$\Delta r$、$\Delta t$ 的灵敏度分析表，提高结论可信度；第三，在获得潜热或经验耦合参数后，可把蒸发潜热作为源项加入温度方程，进一步刻画烘干后期的热湿耦合；第四，对问题四补充物质型与空间型 front-fixing 口径的对照，量化移动边界建模选择对终止时间的影响。
...
# code/problem4/solver.py
# solve_problem4(environment, radius_series, config)
# Landau front-fixing: xi=r/R(t) 物质坐标, xi 系压缩扩散,
# 附录4 物性, 每步 Picard 联立 T/C, R(t) 由附件2 插值,
# 直到 max(C) < 0.15, 输出 t_end, result4.xlsx 与表6。
\end{lstlisting}
