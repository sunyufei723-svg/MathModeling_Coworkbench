...
\end{equation}
连续事件函数取式(26)中的 $g_h(t)$，设置 \texttt{terminal=True}、\texttt{direction}$=-1$。积分器在步内定位
\begin{equation}
  g_h(t^*)=0,
\end{equation}
...
\end{equation}
由式(32)得到首个 $60\rm\,s$ 输出网格达标时刻
\begin{equation}
  t_{60}=205860\rm\,s=57.1833\rm\,h.
\end{equation}
因此，正文报告连续临界时间 $t^*$，结果文件末行采用 $t_{60}$。表\ref{tab:q3-moist} 中终止行中心值显示为 $0.1500$，来源于四舍五入；全精度值满足式(34)。
...
\end{equation}
图\ref{fig:q3-event} 中，$C=C_{\rm cr}$ 等值线由表面向中心收缩，进一步说明干燥前沿自外向内推进；连续临界时间 $t^*$ 和离散输出时刻 $t_{60}$ 分别对应式(33)和式(35)。
...
\end{equation}
可见，边界外推带来的小时级变化显著大于数值离散误差。因此，$57.1727\rm\,h$ 是末值外推边界下的点估计，式(38)用于描述外部边界不确定性。
