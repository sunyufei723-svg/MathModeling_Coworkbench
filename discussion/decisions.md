...
>
> **分区（D7 起）**：编号连续追加，只按阶段归类——「一、前期准备 · 协作规则演进」= 开赛前对协作协议本身的搭建/迭代（本质是改 rules，D0-D7 属此类）；「二、正式阶段 · 业务/比赛决定」= 正式比赛/任务的具体业务决策（暂无）。

---

## 一、前期准备 · 协作规则演进（改 rules，非正式比赛决定）

> 下列 D0-D7 都是对协作规则/协议本身的搭建与优化（读写锁、前置检查、状态优先、合并 push、5 层重构、本文件分区），属正式比赛/任务开始前的准备工作，非业务内容决定；业务决策见文末「二」。
...
> （暂无。正式比赛/任务开始后，针对具体业务内容的决策在此按 D 编号继续追加。）
