...
- **⚠️ 更正（2026-09-12T01:00Z）**：队员质疑「会不会是 R(t) 拟合有问题 / 会不会是含水率算错了」后重新核查：两个质疑都不成立，但发现 `limitations_fix_analysis_by_A.md` 旧 §1.4 的反演论证不自洽、**已作废重写**为更强的**上界论证**（附录 4 密度律的干基密度上确界 760 kg/m³ ＜ 闭合所需 774.26 ⇒ **要闭合就得让含水率变负**；临界半径 R_c=1.2112 cm，附件 2 自 19.5 h 起不可达）。下方**决策 F 的论据已同步更新**（原「三点」→「四点」，旧数值「2 h 处差 +0.66 kg/kg」作废）。
- **⚠️ 补记（2026-09-12T01:32Z）**：队员追问「算上潜热 28.02% 会不会消失」，已验证并补入 `limitations_fix_analysis_by_A.md` §2.1：**不会，反而变大**（硬上界 κ(t\*)≤0.9816 永不闭合；蒸发冷却代理 ΔT=5K 时变幅 28.02%→**30.16%**）。据此**决策 H 已由队员定向：写进论文作为「不考虑潜热」的依据**（论文段落草案见 §2.2）。
...
| D（28.02% 写入评价章） | ✅ 已按「建议写」落地一句话；现提议升级为四点写法（转 **F**） |
| E（第 3 天分工） | ⏳ 仍待协商 |

---

**⭐ 决策 F：Q4 干物质 28.02% 的论文表述要不要从「一句话」升级为「归因 + 不可能性 + 机制 + 定界」四点？**

- **背景**：现行只写了「题目给定 R(t) 与经验密度关系未形成严格质量闭合，是模型局限而非求解器失败」。本轮已把这句话背后的东西全算出来了（详见 `limitations_fix_analysis_by_A.md` §1.6）。
- **升级后的四点**：① **归因**——κ=G·S 精确分解，κ_min=0.7198 @ t≈7 h 就是全部 28.02%（N=401 与 N=3201 只差 1×10⁻⁶ ⇒ 零数值贡献），矛盾两端是 ⟨ρ_s⟩=689.30 vs 闭合要求 774.26 kg/m³，缺口 **11.0%**；② **不可能性（本轮新增的最强论据）**——附录 4 蕴含的干基密度 ρ_s(C)=90+670/(1+C) 在 C≥0 上**上确界仅 760 kg/m³（C=0 绝干）**，而闭合要求 ρ_s=ρ_{s0}(R₀/R)² ⇒ **临界半径 R_c=1.2112 cm**；附件 2 自 **t=19.5 h（R=1.2110 cm）起低于 R_c，此后 106/145 个点全部落入「需负含水率」的不可达区**（t\* 处需 C=−0.0208）；等价地，S 的硬上界 2.7266 ＜ 闭合所需 2.7778 ⇒ κ(t\*) 理论上限仅 **0.9816**，**1.84% 是数学上不可跨越的硬缺口**；③ **机制**——附件 2 的 R 在 20–72 h 只缩 1% 而同期模拟 ⟨C⟩ 降 55%，即**后期收缩与失水完全脱钩**；前期则收缩跑在干燥前面（谷底 7 h 处要闭合需 ⟨C⟩ 再降 72.2%），物理上对应恒速期毛细压实/表面硬化；④ **定界**——强制闭合轨迹下 t\*=60.7312 h（+19.45%），故 **t\* 对收缩律口径的敏感度远高于对边界条件（±6.5%）和数值口径（0.06%）**。
- **两个已被排除的嫌疑（可直接写进论文作防御）**：**R(t) 拟合口径**——附件 2 是 145 个等间隔 0.5 h、严格单调的点，四种口径（linear/PCHIP/三次样条/4 参数指数最小二乘）下 t\* 仅散布 **0.05 h**、变幅 28.00–28.58%，**比 28% 小两个数量级**；**干燥速率**——D 放大 10 倍（t\* 降到 11.98 h）后 κ_min 仍仅 0.847，h_m 放大 20 倍对 t\* 几乎无影响（⇒ 内部扩散控制）。
- **代价**：约 6 行正文 + 1 个小表；需 **files 锁**（改 tex）+ XeLaTeX 重编一次。
- **我的建议**：**做（优先级高于上一轮）**。有了②之后，评委若追问「你既然知道不守恒，为什么不用质量守恒的半径？」，可以直接回答：**「因为按题目给定的密度律，质量守恒要求负含水率——临界半径 1.2112 cm，而附件 2 自 19.5 h 起就低于它。」**——完全可由题给数据验证，比原来那句话有力得多。
- **不做的后果**：不报错，但 28.02% 这个数字在论文里仍是「只承认、未解释」，且我们手上最硬的一条论据（上界证明）没用上。
...
- **新增依据（2026-09-12）**：队员追问「算上潜热 28.02% 会不会消失」——已验证**不会、反而变大**（详 `limitations_fix_analysis_by_A.md` §2.1）：① 硬上界——κ=G·S 中 G 由题给 R(t) 定、S≤760/278.73=2.7266 ⇒ **κ(t\*)≤0.9816，终点缺口至少 1.84% 永不闭合**；② 方向——潜热降温使 D下降、干燥变慢、C 更高、S 更低，蒸发冷却代理 ΔT=0/3/5/10K 下变幅 **28.02%→29.29%→30.16%→32.29%** 单调上升（ΔT=0 逐位复现基线作自检）。即潜热只恶化质量闭合，不是解药。
- **落地形式**：直接用 §2.2 的论文段落草案（能量预算 41 倍 + 计入潜热后不降反升 + κ(t\*)≤0.9816 硬上界）；总模型评价章写一次即可（适用 Q1–Q4）。**必须标明 L_v 是文献值、不属于题给参数**，41 倍与变幅上升均为**量级估算而非结果**。
- **代价 / 锁**：需 **files 锁**。⚠️ **当前 agent_B 正持 `files/final/` 写锁（2026-09-12T01:29:17Z）**，待 B 释锁后写入或与之协调，避免同文件冲突。
...
## [agent_C 2026-09-12T02:02:38Z] 队员对决策 C/E/G/H/I 的确认意见

### 决策 C：问题四不同计算方案在正文中的保留方式——已完成

**决定**：论文正文不再保留 L1-L4 阶梯表，也不再使用 L1、L2、L3、L4 系列编号。正文只保留两个具有实际解释价值的模型，并统一以模型名称指代：

1. **物质坐标 FVM-BDF 模型**：作为问题四主模型，连续临界时间为
   \[
   t^*=50.8245\rm\,h.
   \]
2. **Euler 空间坐标模型**：连续临界时间为
   \[
   t^*=52.3842\rm\,h.
   \]
   该模型只用于考察坐标处理方式对结果的敏感性，**不能作为问题四的答案**。

原 L1 属于早期粗网格计算，只反映求解方案的迭代过程，正文不再提及。原 L2 不再单独列出名称、结果或计算目录，仅保留其对主结果的数值互证作用，并在数值验证部分加入一句：

> 作为数值实现互证，较简化的固定步后向欧拉-Picard 方案与 FVM-BDF 主模型所得临界时间仅相差 \(0.0301\rm\,h\)（\(0.06\%\)），表明主结果对数值实现的敏感性较低。
...
| Euler 空间坐标模型 | 52.3842 | 敏感性对照，不能作为题目答案 |

表题建议改为“问题四两种坐标模型的结果比较”，删除“层级”“代码目录”“L1-L4 阶梯”等开发过程信息。摘要、问题四正文、模型检验、模型评价和结论均使用上述模型名称，不再使用系列编号。

**理由**：L1、L2 属于数值方案的迭代与互证过程，后两个模型属于坐标解释差异，四者不在同一比较维度。删除完整阶梯可以避免出现多个候选答案和内部开发过程，同时通过一句数值互证说明保留必要的可信度证据。仓库内部数值验证文档仍可保留 L1-L4 编号，便于复现和追溯；本决定只约束正式论文正文。

**当前落地状态**：本次只归档并广播意见，**未修改** `files/final/A题完整论文.tex` 或 PDF。当前论文仍保留原 L1-L4 表和系列编号，等待其他 agent 认领后按本决定修改、编译和复核。

### 其他决策同步

- **决策 E**：忽略，不再作为模型或论文待办追踪。
- **决策 G**：时间允许再正式实施。若实施，必须在独立目录以 N=3201 复算；完成前不得把 60.7312 h 列为正式结果，也不得替代题设半径条件下的 50.8245 h 主答案。
- **决策 H**：可以在论文中提及。必须说明潜热参数来自外部文献，41.1 倍等数字属于量级分析，不能用于直接修正 50.8245 h；适合与决策 F 在模型评价中统一处理。
- **决策 I**：已由 agent_B 在提交 `18ea24b` 完成分层 UQ 说明和测试。边界 LHS 只表示边界条件层，不代表问题四全部不确定性；60.7312 h 在正式 N=3201 复现前只能作为诊断值。
- **决策 D/F**：作为同一项“问题四质量闭合局限的论文表述”由 agent_B 归档和处理；agent_C 不重复编辑该段，后续与 H 的论文落地协调完成。

@agent_A @agent_B：请阅读本追加记录并认领正式论文修改。认领者需明确删除 L1-L4 正文编号和四行阶梯表、采用上述两个模型名称，并写明 Euler 空间坐标模型仅为敏感性对照、不能作为答案。
