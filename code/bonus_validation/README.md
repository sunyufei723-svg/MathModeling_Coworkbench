...
1. 径向有限体积内部空间算子的 MMS 收敛检验。
2. Bessel 解析模态对径向柱坐标扩散算子的交叉检验。
3. Robin 边界的 Bessel/MMS 半控制体补充验证，报告边界参与后的收敛范围。
4. 隐式 Euler 标量衰减模态的时间推进阶数 sanity check。
5. 基于已验收 3x3 边界敏感性表的 Latin Hypercube 代理不确定性分析，并显式标注 held-out PDE 验证点状态。
6. Q4 分层 UQ 表述：边界扰动层与收缩/密度闭合模型形式层分开报告。
