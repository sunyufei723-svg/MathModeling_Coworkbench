...
@dataclass(frozen=True)
class SolverConfig:
    xi_points: int = 201                 # ξ∈[0,1] 固定计算网格点数（front-fixing）
...
def _face_arithmetic_average(values: np.ndarray) -> np.ndarray:
    return 0.5 * (values[:-1] + values[1:])


def _face_harmonic_average(values: np.ndarray) -> np.ndarray:
    safe_values = np.maximum(values, 1e-30)
    return 2.0 * safe_values[:-1] * safe_values[1:] / (safe_values[:-1] + safe_values[1:])


def _face_average(values: np.ndarray) -> np.ndarray:
    return _face_arithmetic_average(values)
...
def _implicit_radial_variable_step(
    ...
    """守恒径向 FVM-BDF1 全隐式一步（中心对称消 1/r 奇点、表面第三类 Robin）。
    ...
    n = len(values)
    radius_m = (n - 1) * dr_m
    if face_average == "arithmetic":
        face = _face_arithmetic_average(conductivity)
    elif face_average == "harmonic":
        face = _face_harmonic_average(conductivity)
    else:
        raise ValueError(f"unsupported face_average: {face_average}")
    lower = np.zeros(n - 1)
    diag = np.ones(n)
    upper = np.zeros(n - 1)
    rhs = values.copy()
