"""论文正文图1–图6（脚本内 fig1–fig5）的可复现绘图脚本（单文件 · 无参数运行）。

数据一律取自已提交的结果文件，不新跑物理模型：
- 图1 result1.xlsx（温度 / 水分浓度 两 sheet，t=1..1800 s）——问题一预热阶段径向演化
- 图2 result2.xlsx（温度 / 水分浓度 两 sheet，t=1..10800 s）——问题二变物性热湿耦合时空演化
...
用法：python figures.py   （无任何命令行参数；缺输入文件时给出友好提示后退出）
"""
...
BASE = Path(__file__).resolve().parent          # 本脚本与 resultN.xlsx 同目录（code/submission）
REPO = BASE.parents[1]                           # 仓库根
OUT = BASE / "paper_figures"                     # 图片输出目录
...
def load_field(xlsx: Path, sheet: str | None = None):
    """读 (时间/s 一维, 半径/cm 一维, 场矩阵[nt, nr])；'药材表面' 列单独返回。
    sheet=None（或给定名称不存在）时取第一个工作表——problem3/4 的结果文件为单 sheet。"""
    wb = openpyxl.load_workbook(xlsx, read_only=True, data_only=True)
    ws = wb[sheet] if (sheet is not None and sheet in wb.sheetnames) else wb.worksheets[0]
...
def fig3() -> None:
    p = BASE / "result3.xlsx"
    tt, rr, C, _ = load_field(p)
    ver = json.loads((BASE / "verification3.json").read_text(encoding="utf-8"))
...
def fig4() -> None:
    ...
    from problem4 import ATTACHMENT1, ATTACHMENT2, load_inputs  # 复用求解同口径的半径插值

    env, radius = load_inputs(ATTACHMENT1, ATTACHMENT2)
    p = BASE / "result4.xlsx"
    tt, rr, C, surface = load_field(p)
    ver = json.loads((BASE / "verification4.json").read_text(encoding="utf-8"))
