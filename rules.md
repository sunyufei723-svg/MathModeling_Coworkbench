...
- 请别人做事：在自己文件「发出请求 · 活跃」区 `@目标`（不改对方文件）；请求区分「活跃 / 已闭环归档」两段。
...
_最后更新：2026-09-10（D9：通信层闭环改「发起方追踪制」+ 请求区活跃/归档分离）。修改本协议前，先在 `discussion/decisions.md` 记录共识，再改本文件并通知全部 agent。_
